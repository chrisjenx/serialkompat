...
public class SerialkompatPlugin : Plugin<Project> {
    ...
    private fun runCheck(
        ...
        val baselineText =
            git.snapshotAt(baselineRef, worktrees, cache) { worktreeDir ->
                extractInWorktree(project, worktreeDir)
            }
        ...
        project.logger.lifecycle(outcome.console)
        project.layout.buildDirectory
            .file("serialkompat/report.json")
            .get()
            .asFile
            .also { it.parentFile.mkdirs() }
            .writeText(outcome.json)
        if (outcome.failed) {
    ...
    private fun extractInWorktree(
        ...
        val launcher = File(project.rootDir, gradlew).takeIf(File::exists)?.absolutePath ?: "gradle"
        val process =
            ProcessBuilder(launcher, "${project.path}:$EXTRACT_TASK_NAME", "--quiet")
                .directory(worktreeDir)
                .redirectErrorStream(true)
                .start()
        val log = process.inputStream.bufferedReader().use { it.readText() }
        check(process.waitFor() == 0) { "serialkompat: baseline extraction failed in worktree:\n$log" }
        val relative = project.projectDir.relativeTo(project.rootDir)
